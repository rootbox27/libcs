...
/* Clang has no __builtin_va_arg_pack: use macros, which still see the
 * object size at the call site. */
#define snprintf(d, n, ...) __builtin___snprintf_chk(d, n, 1, __bos(d), __VA_ARGS__)
#define sprintf(d, ...) __builtin___sprintf_chk(d, 1, __bos(d), __VA_ARGS__)
#else

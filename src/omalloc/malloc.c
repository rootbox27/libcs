...
/* CITADEL: OpenBSD's headers are replaced by compat.h. */
#include "compat.h"
...
static union {
	...
	u_char _pad[MALLOC_PAGESIZE];
		...
		/* CITADEL: no .openbsd.mutable section */;
...
find_chunknum(struct dir_info *d, struct chunk_info *info, void *ptr, int check)
{
	...
	/* CITADEL: a pointer into the middle of a chunk is not the chunk */
	if (((uintptr_t)ptr & MALLOC_PAGEMASK) !=
	    chunknum * B2ALLOC(info->bucket))
		wrterror(d, "modified chunk-pointer %p", ptr);
	...
	if (newsz > MALLOC_MAXCHUNK && oldsz > MALLOC_MAXCHUNK && !forced) {
		...
		if (rnewsz > roldsz) {
			...
			if (!mopts.malloc_guard) {
				...
				/* CITADEL: where MAP_FIXED_NOREPLACE is only a
				 * hint (Linux < 4.17), the pages may land
				 * elsewhere; do not leak them. */
				if (q != MAP_FAILED)
					munmap(q, needed);
			}
...
aligned_alloc(size_t alignment, size_t size)
{
	...
	/* CITADEL: C17 (DR 460) dropped the rule that size must be a
	 * multiple of alignment; glibc and musl accept any size too. */
...
/*
 * CITADEL: additions, not part of OpenBSD's malloc.c.
 *
 * malloc_usable_size reports the size that was asked for (not the chunk
 * size): writing up to it never touches a canary or a guard page.
 */
size_t
malloc_usable_size(void *ptr)
{
	struct dir_info *d, *pool;
	struct region_info *r;
	const char *saved_function;
	size_t sz;
	int saved_errno = errno;
	...
	r = findpool(ptr, d, &pool, &saved_function);
	REALSIZE(sz, r);
	if (sz > MALLOC_MAXCHUNK) {
		sz -= mopts.malloc_guard;
	} else if (mopts.chunk_canaries && sz > 0) {
		struct chunk_info *info = (struct chunk_info *)r->size;
		uint32_t chunknum = find_chunknum(pool, info, ptr, 0);

		sz = info->bits[info->offset + chunknum];
	}
	if (pool != d)
		pool->func = saved_function;
	pool->active--;
